...
char render(const int brightness) {
	return ".,-~:;!=*#$@"[brightness];
}
    ...
    for (;;) {
        ...
        for (float theta = 0; theta < two_pi; theta += delta_theta) {
            ...
            for (float phi = 0; phi < two_pi; phi += delta_phi) {
                ...
                quaternion normal = {
                    0, cos_phi * cos_theta * rad_sq_two_plus_cos_theta,
                    (float) pow(cos_phi, 3) * radius_squared,
                    (sin_theta * sin_phi + cos_theta * cos_phi) *
                        rad_sq_two_plus_cos_theta};
                normal = rotate(normal, rotation);
                const int brightness = (int) 
                    8 * sqrt(2) * axial_cos(normal, light_source);
                if (brightness >= 0) {
                    quaternion position = {0, rad_two_plus_cos_theta * cos_phi,
                                           rad_two_plus_cos_theta * sin_phi, z};
                    ...
                    const unsigned int x = (unsigned int) half_display *
                                                   (1 + position.x * z_inverse),
                              		   y = (unsigned int) half_display *
                                                   (1 + position.y * z_inverse);
        ...
        for (unsigned int i = 0; i < display_size; ++i) {
            for (unsigned int j = 0; j < display_size; ++j) {
