    ...
    for (;;) {
        ...
        for (float theta = 0.0f; theta < 2.0f*M_PI; theta += 0.05f) {
            ...
            for (float phi = 0.0f; phi < 2.0f*M_PI; phi += 0.05f) {
                ...
                if (brightness >= 0) {
					Quaternion position = { 0.0f, a0 * cos_phi, a0 * sin_phi, z };
					position = rotate(position, rotation);
					const float z_buffer = position.z - FIVE_RADIUS;
					const float z_inverse = 1.0f / (position.z + FIVE_RADIUS);
